...
def generate_dialogue(chapter_num: str, chapter_title: str, sections: list) -> list:
    ...
    for attempt in range(3):
        ...
        try:
            ...
            print(f"  JSONパースエラー (試行{attempt+1}/3): {e}")
            if attempt < 2:
                time.sleep(3)
    raise ValueError(f"章{chapter_num}: 3回試行後もJSONパース失敗")
...
def tts_voicevox(text: str, speaker_id: int, out_path: Path):
    ...
    for attempt in range(3):
        try:
            query = requests.post(f"{base}/audio_query", params={"text": text, "speaker": speaker_id}, timeout=30).json()
            wav = requests.post(f"{base}/synthesis", params={"speaker": speaker_id}, json=query, timeout=60).content
            out_path.write_bytes(wav)
            return
        except Exception as e:
            print(f"  VOICEVOX接続エラー (試行{attempt+1}/3): {e}")
            if attempt < 2:
                time.sleep(5)
    raise ConnectionError(f"VOICEVOX: 3回試行後も失敗 speaker={speaker_id}")
